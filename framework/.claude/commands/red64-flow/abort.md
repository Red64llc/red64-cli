---
description: Abort a Red64 flow and cleanup resources
...
1. **Check flow exists**:
   - Check if `worktrees/$1` exists OR `.red64/specs/$1/flow-state.json` exists
...
```bash
gh pr close feature/$1 --comment "Aborted via red64-flow"
```
...
```bash
mv .red64/specs/$1/flow-state.json .red64/specs/$1/flow-state.aborted.json
```

Or optionally ask user:
- "Archive the spec artifacts or delete them?"
- Options:
  - "Archive" - Keep in .red64/specs/$1/ with .aborted suffix
  - "Delete" - Remove entire .red64/specs/$1/ directory
