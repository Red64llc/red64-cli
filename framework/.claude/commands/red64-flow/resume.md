---
description: Resume a paused or interrupted Red64 flow
...
1. **Check worktree exists**:
   ...
   ```
   - If not found, report error: "No worktree found for '$1'. Use 'red64-flow list' to see active flows."

2. **Check flow-state.json exists**:
   - Read `.red64/specs/$1/flow-state.json`
...
| Current State | Resume Action |
|---------------|---------------|
| `phases.gap_analysis.completed = false` | Run gap analysis (`/red64:validate-gap`) |
...
| Current State | Resume Action |
|---------------|---------------|
| `phases.design_validation.completed = false` | Run design validation (`/red64:validate-design`) |
...
Execute the appropriate phase from `/red64-flow:start`:
